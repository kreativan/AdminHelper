 ...
 * Classes: PDF 
 * ======================================================
 * This class required mPDF in lib folder
 * which is not included by default
 * ======================================================
 * @method file2pdf($file_path, $vars = [], $options = [])
 * @method html2pdf($html, $options = [])
 * @method generatePDF($pdf_pages = [], $pdf_files = [], $options_arr = [])
 */
...
class PDF extends WireData {
  ...
  public function options($array = []) {
    ...
    $options['fontDir'] = array_merge($fontDirs, [
      $this->AdminHelper->path() . 'lib/fonts/',
    ]);
  ...
  public function html2pdf($html, $params = []) {
    ...
    // mPDF
    require_once($this->AdminHelper->path() . "lib/mpdf/vendor/autoload.php");
    ...
    // Write html
    $stylesheet = file_get_contents($this->AdminHelper->path() . 'lib/css/mpdf.css');
  ...
  public function generatePDF($pdf_pages = [], $pdf_files = [], $options_arr = []) {
    ...
    // mPDF
    require_once($this->AdminHelper->path() . "mpdf/vendor/autoload.php");
    ...
    // Write html
    $stylesheet = file_get_contents($this->AdminHelper->path() . 'css/mpdf.css');
    $mpdf->WriteHTML($stylesheet, \Mpdf\HTMLParserMode::HEADER_CSS);

    $i = 0;

    // Add pages - from $pdf_pages array, by specified (tmpl) template files
